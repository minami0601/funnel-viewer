...
export async function getFunnelData(): Promise<FunnelSection[]> {
  ...
  const createFunnelSteps = (
    ...
    const steps = skipFriends
      ? baseSteps
      : [{ key: `${prefix}友だち数`, label: "友だち" }, ...baseSteps];
    ...
    const funnelData = steps.map((step, index) => {
      ...
      return {
        ...
        percentage:
          currentValue === 0
            ? 0
            : Math.min((nextValue / currentValue) * 100, 100),
      };
    });
  ...
  const createTotalFunnelSteps = (): FunnelData[] => {
    const steps = [
      ...
      {
        key: "日程調整",
        labels: ["新規コンサル日程調整済", "既存コンサル日程調整済"],
      },
    ...
    const funnelData = steps.map((step, index) => {
      const currentValue = step.labels.reduce(
        (sum, label) => sum + (Number(latestData[label]) || 0),
        0,
      );
      if (index === steps.length - 1) {
        return {
      ...
      const nextStep = steps[index + 1];
      const nextValue = nextStep.labels.reduce(
        (sum, label) => sum + (Number(latestData[label]) || 0),
        0,
      );
