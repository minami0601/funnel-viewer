...
interface FunnelStepProps {
  ...
  isLast?: boolean;
  className?: string;
  variant?: 'total' | 'new' | 'existing' | 'company' | 'freelance' | 'experience' | 'salary';
  index?: number;
}

export default function FunnelStep({ data, isLast = false, className = '', variant = 'new', index = 0 }: FunnelStepProps) {
  const getColors = () => {
    switch (variant) {
      case 'total':
        // 全体は赤系統
    ...
    <div className={`flex flex-col min-w-fit ${className}`}>
      <div className="flex items-center">
        <div className="flex flex-col items-center">
          <div className={`w-32 h-32 ${bg} text-white flex items-center justify-center text-center p-3 rounded-lg shadow-md transition-shadow hover:shadow-lg`}>
            <div>
              <div className="font-medium text-sm mb-2 opacity-90">{data.step}</div>
